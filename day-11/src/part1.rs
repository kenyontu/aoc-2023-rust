...
pub fn solve(input: &str) -> u32 {
    ...
    galaxies.iter_mut().for_each(|g| {
        let add_x = cols_to_expand.iter().filter(|c| g.pos.x > **c).count();
        let add_y = rows_to_expand.iter().filter(|c| g.pos.y > **c).count();
    ...
    let mut pairs = 0;
    let sum: usize = (0..galaxies.len())
        .map(|i| {
            (i + 1..galaxies.len())
                .map(|j| {
                    pairs += 1;
                    calc_shortest_dist(&galaxies[i].pos, &galaxies[j].pos)
                })
                .sum::<usize>()
        })
        .sum();
